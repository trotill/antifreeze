...
export async function login(ctx){
    ...
    let token=null
    let err=!await authService.checkLogin({password:ctx.request.body.password,login:ctx.request.body.login,modelDb:ctx.inject.modelDb})
    if (!err){
...
export async function needToken(ctx,next){
    if (ctx.request.header.token) {
        ...
        let tokenFromClient = JSON.parse(ctx.request.header.token);
        const {token,login,error}=await authService.checkToken({token:tokenFromClient,modelDb})
        if (error) {
            ctx.status = 511;
        }
        else {
